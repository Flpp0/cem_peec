...
%% Load frequency data
data = readmatrix('ExportsCOMSOL/V_1C_Original_Dim_Loss.xlsx'); 
end_freq_idx = length(data); % Use all available frequencies
...
for freq_idx = 1:length(frequencies)
    ...
    for config_idx = 1:numConfigurationsMethod1
        ...
        % Voltage stimulation: set terminal voltage V=1
        V_method1 = 1; 
        C_method1 = ones(N_method1,1);
        i_lambda_method1_direct = Z_Lambda_method1 \ (C_method1 * V_method1);
        
        % Calculate total losses and error
    ...
    for config_idx = 1:numConfigurationsMethod2
        ...
        % Calculate Inductance matrix (vectorized approach for Method 2)
        ...
        % Voltage stimulation for Method 2: terminal voltage V=1
        V_method2 = 1;
        C_method2 = ones(N_method2,1);
        i_lambda_method2_direct = Z_Lambda_method2 \ (C_method2 * V_method2);
...
saveDirectory = 'V_1C_Original_Dim_Current_Distribution_Fixed_Mesh/Images_Method1_and_Method2/';
...
for freq_idx = 1:numFrequencies
    ...
    for config_idx = 1:numConfigurationsMethod1
        plot(elements_vec_method1_all{freq_idx}(config_idx), P_total_method1_direct_all{freq_idx}(config_idx), '-^', 'LineWidth', 2, ...
             'DisplayName', ['Method 1 Direct (Config ' num2str(config_idx) ')']);
    end
    % Plot Method 2 results
    for config_idx = 1:numConfigurationsMethod2
        plot(elements_vec_method2_all{freq_idx}(config_idx), P_total_method2_direct_all{freq_idx}(config_idx), '-s', 'LineWidth', 2, ...
             'DisplayName', ['Method 2 Direct (Config ' num2str(config_idx) ')']);
    end
    ...
    subplot(1, 2, 2);
    hold on;
    % Plot Method 1 errors
    for config_idx = 1:numConfigurationsMethod1
        plot(elements_vec_method1_all{freq_idx}(config_idx), errors_vec_method1_direct_all{freq_idx}(config_idx), '-^', 'LineWidth', 2, ...
             'DisplayName', ['Method 1 Direct Error (Config ' num2str(config_idx) ')']);
    end
    % Plot Method 2 errors
    for config_idx = 1:numConfigurationsMethod2
        plot(elements_vec_method2_all{freq_idx}(config_idx), errors_vec_method2_direct_all{freq_idx}(config_idx), '-s', 'LineWidth', 2, ...
             'DisplayName', ['Method 2 Direct Error (Config ' num2str(config_idx) ')']);
    end
    xlabel('Number of Elements');
    ...
    if saveAndClose
        ...
        saveas(fig, fullfile(saveDirectory, ['Losses_Convergence_', num2str(f/1e6), 'MHz.png']));
        close(fig);
    end
end
...
% Plot Errors for Method 1 and Method 2
subplot(3, 1, 2);  
hold on;
for config_idx = 1:numConfigurationsMethod1
    errors_final_method1_direct = abs(cellfun(@(x) x(config_idx), P_total_method1_direct_all) - reference_losses_list);
...
% Plot Percentage Errors for Method 1 and Method 2
subplot(3, 1, 3);
hold on;
for config_idx = 1:numConfigurationsMethod1
    errors_final_method1_direct = abs(cellfun(@(x) x(config_idx), P_total_method1_direct_all) - reference_losses_list);
